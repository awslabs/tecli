...
func SetConfigurationVersionFlags(cmd *cobra.Command) {
	...
	usage := `When true, runs are queued automatically when the configuration version
	is uploaded.`
	cmd.Flags().Bool("configuration-version-auto-queue-runs", false, usage)

	// usage = `TODO`
	// cmd.Flags().String("configuration-version-error", "", usage)

	// usage = `TODO`
	// cmd.Flags().String("configuration-version-error-message", "", usage)

	usage = `When true, this configuration version can only be used for planning.`
	cmd.Flags().Bool("configuration-version-speculative", false, usage)

	// usage = `TODO`
	// cmd.Flags().String("configuration-version-upload-url", "", usage)
}
